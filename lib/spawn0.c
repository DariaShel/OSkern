...
map_segment(envid_t child, uintptr_t va, size_t memsz,
    ...
    if (memsz > filesz) {
        if (sys_alloc_region(child, (void *) va + filesz, memsz - filesz, perm)) {
            return -E_NO_MEM;
        }
    }
    
    /* Allocate filesz in parent to UTEMP */
    if (sys_alloc_region(CURENVID, UTEMP, filesz, PROT_RW)) {
        return -E_NO_MEM;
    }
    ...
    //     }
    // }
    readn(fd, UTEMP, filesz);
    
    /* Map read section conents to child */
    if ((res = sys_map_region(CURENVID, UTEMP, child, (void *) va, filesz, perm | PROT_LAZY)) < 0) {
        return res;
    }

    /* Unmap it from parent */
    if ((res = sys_unmap_region(CURENVID, UTEMP, filesz)) < 0) {
        return res;
    }
